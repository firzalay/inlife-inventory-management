...
The system reaches for color rarely — Signal Red marks links, primary CTAs, and the rare full-bleed CTA banner. Black carries every other surface that isn't white or gray. The result is enterprise gravitas without the enterprise stiffness: rigorous, high-contrast, and intentionally restrained — reading almost like a CMYK print swatch (pure red, pure black, mid gray, and white).
...
- **One accent color**: `{colors.primary}` Signal Red carries every link, primary CTA, and CTA banner. There is no second brand color.
- White canvas + mid gray (`{colors.surface-1}`) + true black (`{colors.ink}`) cover 95% of surfaces — a four-ink palette (Red / White / Gray / Black) instead of a tinted charcoal.
- Footer inverts to true black (`{colors.inverse-canvas}` #000000) — the only dark surface above the page break.
...
> Palette re-based on a 4-color CMYK swatch: Red (C0 M95 Y100 K0), White (C0 M0 Y0 K0), Gray (C0 M0 Y0 K70), Black (C0 M0 Y0 K100).

### Brand & Accent
- **Signal Red** ({colors.primary}, #ff0d00 — C0 M95 Y100 K0): The single brand accent. Links, primary CTAs, CTA banner backgrounds, focus rings.
- **Red Hover** ({colors.red-hover}, #d90b00): Hovered link / button state.
- **Red 80** ({colors.red-80}, #a60900): Pressed primary button.
- **Red Hover** ({colors.blue-hover} → renamed {colors.red-hover-alt}, #d90b00): Hover state for primary buttons (same value as Red Hover above — single hover ramp).

### Surface
- **Canvas** ({colors.canvas}, #ffffff — C0 M0 Y0 K0): Default page background.
- **Surface 1** ({colors.surface-1}, #f4f4f4): Light gray — input fields, alternate-row stripes, subtle section bands.
- **Surface 2** ({colors.surface-2}, #e0e0e0): Slightly darker gray — disabled fields, hairline-as-fill for separators.
- **Hairline** ({colors.hairline}, #e0e0e0): 1px borders on cards, inputs, dividers.
- **Hairline Strong** ({colors.hairline-strong}, #000000): 1px black underline on focused inputs (Carbon's signature focus treatment).
- **Inverse Canvas** ({colors.inverse-canvas}, #000000 — C0 M0 Y0 K100): True black — footer surface.
- **Inverse Surface 1** ({colors.inverse-surface-1}, #262626): One step lighter than inverse canvas — footer column dividers, hovered footer items.

### Text
- **Ink** ({colors.ink}, #000000 — C0 M0 Y0 K100): All headlines and emphasized body type — true black.
- **Ink Muted** ({colors.ink-muted}, #4d4d4d — C0 M0 Y0 K70): Secondary type — meta, sub-headlines, footer body. This is the swatch gray from the reference image.
- **Ink Subtle** ({colors.ink-subtle}, #8c8c8c): Tertiary type — disabled, helper text, captions.
- **Inverse Ink** ({colors.inverse-ink}, #ffffff): White on black — footer headings.
- **Inverse Ink Muted** ({colors.inverse-ink-muted}, #b3b3b3): Light gray on black — footer body.

### Semantic
- **Success Green** ({colors.semantic-success}): Carbon green-50 — success states. (Unchanged — kept distinct from the red/black/gray/white core palette.)
- **Warning Yellow** ({colors.semantic-warning}): Carbon yellow-30 — warning states. (Unchanged.)
- **Error Red** ({colors.semantic-error}): Carbon red-60 (#da1e28) — error states; danger button background. Kept as a separate, slightly darker/cooler red than `{colors.primary}` so error states remain visually distinct from brand CTAs.
- **Info Blue** ({colors.semantic-info}) → **Info Red**: Now identical to `{colors.primary}` (#ff0d00) — informational badges follow the single-accent rule.
...
- **Soft red gradient backdrops** appear behind some hero illustrations — a faint red-to-white wash that warms the canvas without competing with the headline.
...
**`button-primary`** — Red solid CTA. The default primary across all pages.
- Background `{colors.primary}`, text `{colors.on-primary}` (white), type `{typography.button}`, padding 12px 16px, rounded `{rounded.none}`.
- Pressed state lives in `button-primary-pressed` (background shifts to `{colors.red-80}`, #a60900).

**`button-secondary`** — Black solid button — Carbon's "secondary" treatment.
- Background `{colors.ink}` (#000000), text `{colors.inverse-ink}` (white), type `{typography.button}`, padding 12px 16px, rounded `{rounded.none}`.

**`button-tertiary`** — White button with red 1px border + red text. Used for tertiary CTAs.
...
- Background `{colors.semantic-error}` (#da1e28), text `{colors.on-primary}`, type `{typography.button}`, rounded `{rounded.none}`, padding 12px 16px.
...
**`cta-banner`** — Full-width red CTA panel near the bottom of the page.
...
**`footer`** — Black footer (`{colors.inverse-canvas}`) with the IBM wordmark left and 5–6 columns of caption-sized links. The only inverted surface above the page break.
...
- Reserve `{colors.primary}` Signal Red for primary CTAs, links, focused-input underlines, and CTA banner. Do not use it as a card background or eyebrow color.
...
- Invert to `{colors.inverse-canvas}` (true black) only at the footer; the rest of the page stays light.
- Keep the palette to four inks — Red, White, Gray, Black — echoing the CMYK reference swatch. Tint/shade within that family rather than introducing new hues.
...
- Don't add atmospheric depth (gradient backdrops, drop shadows, atmospheric overlays) outside the documented soft-red hero gradient.
- Don't introduce a second brand color. Signal Red is the only chromatic accent; status semantics use the documented green / yellow / error-red.
...
6. Treat Signal Red as scarce: links, primary CTA, CTA banner, focus underline. Anything beyond that is drift.
...
- Semantic Success/Warning colors were left unchanged from the original Carbon palette since the reference CMYK swatch (Red/White/Gray/Black) only defines the neutral + accent ramp, not full status semantics.